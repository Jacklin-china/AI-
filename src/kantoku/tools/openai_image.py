"""OpenAI GPT-Image 同步生图适配器；生成成功后先原子落盘再返回。"""
...
class OpenAIImageProvider:
    ...
    def _validate_configuration(self) -> None:
        ...
        if width % 16 or height % 16:
            raise ConfigError("GPT-Image 输出宽高必须是 16 的倍数")
        if max(width, height) > _MAX_EDGE or not _MIN_PIXELS <= pixels <= _MAX_PIXELS:
            raise ConfigError("GPT-Image 输出尺寸超出支持范围")
        if not 1 / 3 <= width / height <= 3:
            raise ConfigError("GPT-Image 输出宽高比超出支持范围")
    ...
    def validate_request(
        ...
        *,
        ...
        if reference_urls:
            raise ToolError("GPT-Image 参考图需要走图片编辑接口，当前单镜生成暂不接受 URL")
        if seed is not None:
            raise ToolError("当前 GPT-Image 接口不支持固定 seed")
        ...
        *,
        ...
        try:
            ...
            if not isinstance(data, list) or len(data) != 1:
                raise ToolError("GPT-Image 未返回唯一图片，费用需要人工对账")
            encoded = getattr(data[0], "b64_json", None)
            if not isinstance(encoded, str) or not encoded:
                raise ToolError("GPT-Image 返回的图片内容为空，费用需要人工对账")
            try:
                image_bytes = base64.b64decode(encoded, validate=True)
            except (binascii.Error, ValueError) as error:
                raise ToolError("GPT-Image 图片内容无法解码，费用需要人工对账") from error
    ...
    def query(self, provider_job_id: str) -> ImageGenerationResult:
        """同步 API 没有远端任务查询；只读取 submit 已原子保存的结果。"""
