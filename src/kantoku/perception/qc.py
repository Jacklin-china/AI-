...
不要声称存在统一的“大众审美”。
先检查反证，不要因图片清晰或符合关键词就给出全面肯定。逐项留意：表情是否模板化、
视线是否有对象、搀扶和握持是否有可信支撑、人物步态是否同步摆拍、衣物雨水分布是否
符合遮挡、路面反射和背景细节是否有空间逻辑。只描述实际看得见的证据；看不清手部
时必须写“无法确认”，不能写“手指无缺陷”。用户要求写实时，说明最突出的不自然之处
和一个最小修改建议；找不到确证时承认局限，不用一串“自然、合理”替代证据。
不要声称检测到真实摄影或保证没有 AI 味；风格化作品按其目标风格评价。
JSON Schema：{schema}
"""
