...
| 模式 | 入口与体验 | 人工参与 |
|---|---|---|
| Fast Creation（内部交互模式 Autonomous） | 首页默认模式。用户不必先选功能，直接描述需求；也可在当前聊天用“+”选择漫剧、电商或视觉创作，不跳转页面。Agent 判断领域与动作，自动补齐可推断参数并执行可用能力或工作流。 | 只在预算、质量、风险或任务本身需要时打断用户；不追问尺寸、镜头、Prompt 等专业参数。 |
| Professional Creation（内部交互模式 Guided） | 用户主动进入漫剧、电商、广告、Studio 等创作域。系统提供领域上下文、引导提问、专业工作流、中间结果、提示词编辑、QC 与返工。 | 用户可以检查阶段结果并作必要的确认或修改。 |
