...
const props = defineProps<{ disabled?: boolean; fastDomains?: boolean; fastDomain?: FastDomain | null }>()
...
<template>
  <div class="message-composer" @keydown.esc="menuOpen = false">
    <div v-if="fastDomains && fastDomain" class="composer-domain-chip">
      ...
      <button type="button" aria-label="退出快捷模式" @click="selectDomain(null)"><X :size="13" /></button>
    </div>
    ...
    <footer>
      <div v-if="fastDomains" class="composer-domain-control">
        <button type="button" class="composer-tool composer-plus" aria-label="选择创作域快捷模式" :aria-expanded="menuOpen" aria-haspopup="menu" @click="menuOpen = !menuOpen"><Plus :size="18" /></button>
