...
import { cancelCoreBatch, cancelCoreRun, createCoreBatch, decideCoreApproval, getCoreApprovals, getCoreArtifacts, getCoreBatches, getCoreRuns, getCoreSkills, getState, loadArtwork, startAction } from './api'
...
import type { CoreApproval, CoreArtifact, CoreBatch, CoreRun, CoreSkill, GenerationSettings, StudioForm, StudioState, StudioTask } from './types'
...
type RouteName = 'dashboard' | 'workspace' | 'runs' | 'run_detail' | 'approvals' | 'batches' | 'assets' | 'history' | 'skills' | 'settings' | 'domain'
...
const coreRuns = ref<CoreRun[]>([])
const coreApprovals = ref<CoreApproval[]>([])
const coreArtifacts = ref<CoreArtifact[]>([])
const coreSkills = ref<CoreSkill[]>([])
const coreBatches = ref<CoreBatch[]>([])
...
const runDetail = computed(() => coreRuns.value.find((run) => run.id === route.value.param) ?? null)
...
function quickStart(domain: DomainDefinition): void { activeDomainId.value = domain.id; if (domain.id === 'commerce') { navigate('/batches'); setNotice('Commerce 已接入真实 Core；外部 Source 与 Marketplace 明确使用 Mock。') } else if (domain.status === 'available') { resetWorkspace(); navigate('/workspace') } else { navigate(`/domain/${domain.id}`); setNotice(`${domain.name} 当前为 ${domain.status === 'preview' ? 'Preview' : 'Coming Soon'}，不会伪造执行。`) } }
...
function routeTitle(): string { return ({ dashboard: '首页', workspace: '工作台', runs: '任务与运行', run_detail: '运行详情', approvals: '审批', batches: '批量任务', assets: '资产', history: '历史记录', skills: '技能', settings: '设置', domain: activeDomain.value.name } as Record<RouteName, string>)[route.value.name] }
...
function coreWorkflow(run: CoreRun | null): WorkflowStep[] { return run?.nodes.map((node) => ({ id: node.node_id, name: node.node_id, status: node.status, detail: node.error ?? (node.completed_at ? `完成 · retry ${node.retry_count}` : `retry ${node.retry_count}`) })) ?? [] }
function openCoreRun(run: CoreRun | string): void { const id = typeof run === 'string' ? run : run.id; navigate(`/runs/${encodeURIComponent(id)}`) }
async function cancelRun(run: CoreRun): Promise<void> { if (!(await ask('取消 Run？', `${run.id}\n取消后不会继续执行尚未完成的节点。`, { confirmLabel: '确认取消' }))) return; try { await cancelCoreRun(run.id); await refreshCore(); setNotice('Run 已取消。', 'success') } catch (error) { setNotice(error instanceof Error ? error.message : '取消失败', 'error') } }
async function cancelBatch(batch: CoreBatch): Promise<void> { if (!(await ask('取消批次？', `将取消批次 ${batch.name} 中所有未结束的 Run。`, { confirmLabel: '确认取消' }))) return; try { await cancelCoreBatch(batch.id); await refreshCore(); setNotice('批次已取消。', 'success') } catch (error) { setNotice(error instanceof Error ? error.message : '取消失败', 'error') } }
async function createBatchDemo(): Promise<void> { if (!(await ask('创建 Commerce 5 Run Demo？', '将创建 5 个真实 Core Run 并停在候选审批；外部 Source 与 Marketplace 均明确为 Mock，不产生费用。', { confirmLabel: '创建 Demo' }))) return; coreMutating.value = true; try { await createCoreBatch({ name: 'Commerce 5 Run Demo', workflow: 'commerce.production.v1', concurrency_limit: 2, items: ['便携阅读灯', '保温水杯', '桌面收纳架', '旅行颈枕', '无线鼠标'].map((requirement) => ({ requirement, locale: 'ru-RU' })) }); await refreshCore(true); setNotice('5 个 Commerce Run 已创建，等待候选审批。', 'success') } catch (error) { setNotice(error instanceof Error ? error.message : 'Demo 创建失败', 'error') } finally { coreMutating.value = false } }
async function decideCore(item: CoreApproval, action: 'approve' | 'reject' | 'revise'): Promise<void> { if (!(await ask('提交审批决定？', '该决定会被持久化，并由后端只恢复一次原 Run。', { confirmLabel: action === 'approve' ? '批准并继续' : '提交决定' }))) return; coreMutating.value = true; try { await decideCoreApproval(item.id, action); await refreshCore(true); setNotice('审批决定已保存，Run 已由 Core 继续。', 'success') } catch (error) { setNotice(error instanceof Error ? error.message : '审批失败', 'error') } finally { coreMutating.value = false } }
...
async function refreshCore(_force = false): Promise<void> { const [runs, approvals, artifacts, skills, batches] = await Promise.all([getCoreRuns(), getCoreApprovals(), getCoreArtifacts(), getCoreSkills(), getCoreBatches()]); coreRuns.value = runs; coreApprovals.value = approvals; coreArtifacts.value = artifacts; coreSkills.value = skills; coreBatches.value = batches }
async function refresh(): Promise<void> { if (refreshing.value) return; refreshing.value = true; try { const [snapshot] = await Promise.all([getState(), refreshCore()]); state.value = snapshot; generation.model ||= snapshot.config.image; await handleCompletedJob(snapshot); await Promise.all(snapshot.tasks.filter((task) => task.has_image).slice(0, 12).map(ensurePreview)) } catch (error) { setNotice(`本机服务暂不可用：${error instanceof Error ? error.message : '未知错误'}`, 'error') } finally { refreshing.value = false } }
...
<template>
  <div class="kantoku-shell" :class="{ 'sidebar-collapsed': sidebarCollapsed }">
    <SidebarNav :route-name="route.name" :active-domain-id="activeDomainId" :task-count="coreRuns.length" :collapsed="sidebarCollapsed" @navigate="navigate" @domain="chooseDomain" @toggle="sidebarCollapsed = !sidebarCollapsed" />
    <section class="app-stage"><TopBar :title="routeTitle()" :busy="busy" @command="commandOpen = true" />
      ...
      <RunsView v-else-if="route.name === 'runs'" :runs="coreRuns" @open="openCoreRun" @cancel="cancelRun" @create="navigate('/workspace')" />
      <ApprovalsView v-else-if="route.name === 'approvals'" :approvals="coreApprovals" :runs="coreRuns" :busy="coreMutating" @decide="decideCore" @open="openCoreRun" />
      <BatchesView v-else-if="route.name === 'batches'" :batches="coreBatches" @open="openCoreRun" @cancel="cancelBatch" @create-demo="createBatchDemo" />
      <AssetsView v-else-if="route.name === 'assets'" :artifacts="coreArtifacts" :runs="coreRuns" @open="openCoreRun" />
      <main v-else-if="route.name === 'run_detail'" class="page run-detail-page"><header class="page-header"><div><button class="back-link" @click="navigate('/runs')">← 任务与运行</button><h1>{{ runDetail ? `${runDetail.domain} Production` : '未找到运行' }}</h1><p v-if="runDetail">{{ runDetail.id }}</p></div><StatusBadge v-if="runDetail" :status="runDetail.status" /></header><template v-if="runDetail"><nav class="detail-tabs"><button v-for="item in ([['overview','摘要'],['workflow','节点 Trace']] as const)" :key="item[0]" :class="{ active: runDetailTab === item[0] }" @click="runDetailTab = item[0]">{{ item[1] }}</button></nav><section v-if="runDetailTab === 'overview'" class="surface detail-summary"><header class="panel-header"><div><span class="section-kicker">Core Run</span><h2>执行摘要</h2></div></header><dl><div><dt>创作域</dt><dd>{{ runDetail.domain }}</dd></div><div><dt>工作流</dt><dd>{{ runDetail.workflow }}</dd></div><div><dt>当前节点</dt><dd>{{ runDetail.current_node || '—' }}</dd></div><div><dt>费用</dt><dd>{{ formatFen(runDetail.cost_fen) }}</dd></div><div><dt>节点数</dt><dd>{{ runDetail.nodes.length }}</dd></div><div><dt>产物数</dt><dd>{{ coreArtifacts.filter((item) => item.run_id === runDetail?.id).length }}</dd></div></dl><div v-if="runDetail.error" class="error-box">{{ runDetail.error }}</div><div class="detail-actions"><button v-if="['pending','running','waiting'].includes(runDetail.status)" class="ui-button" @click="cancelRun(runDetail)">取消 Run</button><button v-if="coreApprovals.some((item) => item.run_id === runDetail?.id && item.decision === 'pending')" class="ui-button primary" @click="navigate('/approvals')">处理审批</button></div></section><section v-else class="surface detail-tab-panel"><WorkflowProgress :steps="coreWorkflow(runDetail)" /></section></template><EmptyState v-else title="未找到运行" description="该 Core Run 不存在或已经从本机记录中移除。" /></main>
      ...
      <main v-else-if="route.name === 'skills'" class="page"><header class="page-header"><div><span class="section-kicker">文件化技能注册表</span><h1>技能</h1><p>以下定义由 SkillLoader 从 manifest.yaml 加载，handler 路径保持项目相对路径。</p></div><span class="count-chip">{{ coreSkills.length }} 已加载</span></header><section class="skill-grid"><article v-for="item in coreSkills" :key="item.id" class="surface skill-card"><header><span class="domain-icon">{{ item.domain.slice(0, 2).toUpperCase() }}</span><StatusBadge status="available" /></header><span class="section-kicker">{{ item.domain }} · v{{ item.version }}</span><h2>{{ item.name }}</h2><p>{{ item.description }}</p><footer><span>{{ item.handler_ref ?? '无 Handler' }}</span><div><b v-for="tool in item.required_tools" :key="tool">{{ tool }}</b></div></footer></article></section><EmptyState v-if="!coreSkills.length" title="尚未加载 Skill" description="请检查 skills 目录和 manifest.yaml。" /></main>
